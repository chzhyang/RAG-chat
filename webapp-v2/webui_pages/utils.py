...
class ApiRequest:
    ...
    def create_knowledge_base_v1(
        ...
        '''
        No support for kb management in kb server, just use KB_LIST store the kb info, real kb creat in upload_kb_doc_v1()
        '''
        if no_remote_api is None:
            no_remote_api = self.no_remote_api
    ...
    def delete_knowledge_base_v1(
        ...
        if no_remote_api:
            ...
            KB_LIST = list(filter(lambda kb: kb.get('kb_name') != kb_name, KB_LIST))
            response = {
                "msg": f"Delete {kb_name} successfully"
            }
            return response
    ...
    # v1: also means create a new knowledge base in milvus
